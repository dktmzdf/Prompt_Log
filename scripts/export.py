...
def detect_agent(path, recs=None):
    ...
    for rec in rows[:50]:
        if rec.get("type") == "session_meta" and isinstance(rec.get("payload"), dict):
            return "codex"
        if rec.get("type") in ("user", "assistant") and isinstance(
...
def build(recs):
    ...
    for rec in recs:
        ...
        if said:
            prompts.append(
                {"ts": rec.get("timestamp", ""), "text": said, "via": via, "items": []}
            )
        ...
        for block in msg.get("content") or []:
            ...
            if btype == "thinking" and block.get("thinking", "").strip():
                ...
                events.append(
                    {"kind": "thinking", "ts": rec.get("timestamp", ""), "text": text}
                )
                add(("thinking", text, None))
            elif btype == "text" and block.get("text", "").strip():
                text = block["text"].strip()
                events.append(
                    {"kind": "answer", "ts": rec.get("timestamp", ""), "text": text}
                )
    ...
    for b in buckets.values():
        ...
        if b["pre"]:
            b["prompts"].insert(
                0,
                {
                    "ts": st["times"][0] if st["times"] else "",
                    "text": "",
                    "via": PRE_PROMPT,
                    "items": b["pre"],
                },
...
def build_codex(recs):
    ...
    meta = next(
        (r.get("payload", {}) for r in recs if r.get("type") == "session_meta"), {}
    )
    ...
    for rec in recs:
        ...
        if native:
            ...
            if kind == "usermessage":
                ...
                add_text_event(
                    rec, "answer", codex_text(item.get("content")), item.get("phase")
                )
            elif kind == "reasoning":
                text = codex_text(
                ...
                if output is None:
                    output = "\n".join(
                        x for x in (item.get("stdout"), item.get("stderr")) if x
                    )
                status = normalized_type(item.get("status"))
                ok = item.get("exit_code") in (None, 0) and status not in (
                    "failed",
                    "declined",
                    "cancelled",
                )
                add_tool(
                    rec, "Bash", inp, output, ok, " ".join(command_display.split())[:60]
                )
                ...
                add_tool(
                    ...
                    and normalized_type(item.get("status")) != "failed",
                )
                ...
                add_tool(
                    ...
                    name,
                    item.get("arguments") or {},
                    item.get("contentItems") or item.get("content_items"),
                ...
                add_tool(
                    ...
                    name,
                    inp,
                    item.get("agentsStates") or item.get("receiverThreadIds"),
...
def export(path, agent=None, report_root=REPORT_ROOT):
    ...
    for day in days:
        ...
        for d in (
            Path(report_root),
            Path(report_root) / detected,
            Path(report_root) / detected / slug,
            Path(report_root) / detected / slug / day,
            out_dir,
        ):
        ...
        written.append((base, real_prompts(b["prompts"]), sum(stats["tools"].values())))
    return written
...
def main():
    ...
    if args.all:
        ...
        if "claude" in selected:
            targets.extend(
                (p, "claude") for p in sorted(SOURCE_ROOTS["claude"].glob("*/*.jsonl"))
            )
        if "codex" in selected:
            targets.extend(
                (p, "codex") for p in sorted(SOURCE_ROOTS["codex"].rglob("*.jsonl"))
            )
    elif args.paths:
